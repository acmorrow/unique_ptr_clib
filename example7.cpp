...
namespace libfoo {
    ...
    struct lifecycle_traits<foo_thing1_t> {
        ...
        static constexpr const auto& ctor = foo_thing1_create;
        static constexpr const auto& dtor = foo_thing1_destroy;
        using ctor_type = decltype(ctor);
        using dtor_type = decltype(dtor);
    };

    template<>
    struct lifecycle_traits<foo_thing2_t> {
        using value_type = foo_thing2_t;
        static constexpr const auto& ctor = foo_thing2_create;
        static constexpr const auto& dtor = foo_thing2_destroy;
        using ctor_type = decltype(ctor);
        using dtor_type = decltype(dtor);
    };
